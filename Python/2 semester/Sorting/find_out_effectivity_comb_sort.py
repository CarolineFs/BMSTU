...
def comb_sort(array):
    ...
    :param array: массив
    :return: время сортировки, отсортированный массив
    '''
...
def checker(value, entry):
    '''
    Проверка введенных в поле ввода значения
    :param value: значение поля
    :param entry: поле ввода, значение которого проверяется
    :return: значение поля, в виде строки, если не удалось преобразовать
    в тип int, либо преобразованное в тип int значение
    '''
    try:
        value = int(value)
    except ValueError:
...
def open_warning_window(message):
    ''' Открытие окна с предупреждением message '''
...
def convert_array_to_string(array):
    '''
    Записывает элементы массива в строку через запятую
    :param array: входной массив
    :return: записанные в строку через запятую элементы массива
    '''
    str_array = ''
    for i in array:
        str_array += str(i) + ','
...
def new_example_array(canvas):
    '''
    Генерирует и выводит на холст случайно сгенерированный массив из 10 элементов
    :param canvas: холст
    :return: ничего
    '''
