...
class graph:
    ...
    def choose_points(self, event):
        ...
        min_dif = -1
        flag_min_dif = 0
        flag_out = 1 # = 0, если минимальная разность = 0, выход из всех циклов
        for i in range(len(self.normal_coords)):
            point1 = self.normal_coords[i]
            if not flag_out:
                break
            for j in range(i+1, len(self.normal_coords)):
                point2 = self.normal_coords[j]
                if not flag_out:
                    break
                for k in range(j+1, len(self.normal_coords)):
                    if not flag_out:
                        break
                    point3 = self.normal_coords[k]
                    points_in = 0
                    points_out = 0
                    a = sqrt(((point2[0] - point1[0]) ** 2) + ((point2[1] - point1[1]) ** 2))
                    b = sqrt(((point3[0] - point1[0]) ** 2) + ((point3[1] - point1[1]) ** 2))
                    c = sqrt(((point3[0] - point2[0]) ** 2) + ((point3[1] - point2[1]) ** 2))
                    if a + b > c and \
                        a + c > b and \
                        b + c > a:
                        ...
                        for t in range(len(self.normal_coords)):
                            ...
                            if ((point1[0]-cur_point[0])*(point2[1]-point1[1]) - (point2[0]-point1[0])*(point1[1]-cur_point[1])) > 0 and \
                                ((point2[0]-cur_point[0])*(point3[1]-point2[1]) - (point3[0]-point2[0])*(point2[1]-cur_point[1])) > 0 and \
                                ((point3[0]-cur_point[0])*(point1[1]-point3[1]) - (point1[1]-point3[0])*(point3[1]-cur_point[1])) > 0:
                                points_in += 1
                            elif ((point1[0]-cur_point[0])*(point2[1]-point1[1]) - (point2[0]-point1[0])*(point1[1]-cur_point[1])) < 0 and \
                                ((point2[0]-cur_point[0])*(point3[1]-point2[1]) - (point3[0]-point2[0])*(point2[1]-cur_point[1])) < 0 and \
                                ((point3[0]-cur_point[0])*(point1[1]-point3[1]) - (point1[1]-point3[0])*(point3[1]-cur_point[1])) < 0:
                                points_in += 1
                            elif ((point1[0]-cur_point[0])*(point2[1]-point1[1]) - (point2[0]-point1[0])*(point1[1]-cur_point[1])) == 0 and \
